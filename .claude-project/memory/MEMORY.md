...
- [pytest-httpx 0.36 API](pytest-httpx-036-api.md) — url=re.compile(...) 사용, 응답 재사용 없음(요청 수=등록 수), 병렬 fallback은 searchdate별 regex
...
- [EVCS expand_evcs per-call flag (v0.1.2)](aggregator-evcs-per-call-flag.md) — additive is_virtual 거부, 구조적 double-count 방지 + 금액 행 단위 round-half-even
- [verify_sums org prefix rollup](verify-sums-org-prefix-rollup.md) — 가짜 PASS 제거: 소계 prefix 스코핑 + SKIPPED + pd.isna 명시 처리
- [exchange sanity 전영업일 비교](exchange-sanity-prev-business-day.md) — 세션 _rates_by_date 저장소 + D-1..D-7 병렬 fallback 부산물 재활용
- [workbook_cache df.copy LRU](workbook-cache-df-copy-lru.md) — mtime_ns+size 키 + 방어적 copy + write 후 invalidate
